...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug, city: rawCity, district: rawDistrict } = await params;
  ...
  const description = `${district}, ${city} bölgesindeki ${displayName.toLowerCase()} uzmanlarını UrGlowUp'ta keşfedin.`;
...
export default async function CategoryCityDistrictPage({ params, searchParams }: PageProps) {
  const { slug, city: rawCity, district: rawDistrict } = await params;
  const city = decodeURIComponent(rawCity);
  const district = decodeURIComponent(rawDistrict);
    ...
    <>
      ...
      <div className="container mx-auto space-y-8 px-4 py-10">
        ...
        <nav className="flex flex-wrap items-center gap-1.5 text-sm text-muted-foreground">
          ...
          <ChevronRight className="size-3.5 text-border" />
          <Link href={cityPath} className="hover:underline">{city}</Link>
          <ChevronRight className="size-3.5 text-border" />
        ...
        {businesses.length === 0 && hasAnyFilter ? (
          ...
          <BusinessGrid
            ...
            emptyMessage={`${district}, ${city} bölgesinde ${displayName.toLowerCase()} kategorisinde henüz uzman listelenmedi.`}
          />
        )}
      </div>
    </>
